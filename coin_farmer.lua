-- MM2 Coin Farmer - FIXED FOR RESPAWN (with speed control)
local SPEED = 35  -- change this to go faster (50) or slower (20)
...
-- These will be updated on respawn
local Character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
local Humanoid = Character:WaitForChild("Humanoid")
local Root = Character:WaitForChild("HumanoidRootPart")

-- Refresh function – called on every respawn
...
local function FarmLoop()
    while isFarming do
        -- Wait if character is not loaded
        if not Root or not Root.Parent then
            RefreshCharacter()
            task.wait(0.5)
            continue
